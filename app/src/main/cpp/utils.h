...
char video_directory[PATH_MAX];
char sdcard_directory[PATH_MAX];
...
int list_dir(char *path, dynarray_t *files) {
    DIR *dir = opendir(path);
    if (dir) {
        struct dirent *dp;
        struct stat s;
...
void generateMd5(rapidstring *s, const char *filename) {
...
void handle_api_videos(struct mg_connection *nc, int ev, void *p) {
...
void handle_videos(struct mg_connection *nc, int ev, void *p) {
    // 发送 videos.html 文件
...
JAVA_STATIC_METHOD(CLASS, youdao, jstring, jstring query_,
...
JAVA_STATIC_METHOD(CLASS, google, jstring, jstring query_,
                   jboolean is_english_to_chinese) {
...
JAVA_STATIC_METHOD(CLASS, deleteFileSystem, jboolean,
    ...
    (*env)->
            ReleaseStringUTFChars(env, path_, path
    );
    return ret == 0 ? true : false;
}
